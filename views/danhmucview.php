<?php
    ...
    function createSqlTimKiemDanhMuc(){
        ...
        if(isset($_POST['button_form_timkiem_danhmuc'])){  
            ...
            if(isset($_POST['form_mota']) && $_POST['form_mota'] != ""){
                $sql .= " and mota like '%" . $_POST['form_mota'] . "%'";
            } 
            if(isset($_POST['form_selectmadanhmuc']) && $_POST['form_selectmadanhmuc'] != ""){
                $sql .= " and madanhmuc = '" . $_POST['form_selectmadanhmuc'] . "'";
            }
        }
        return $sql;
    }

    if(isset($_POST['form_madanhmuc'],$_POST['form_tendanhmuc'],$_POST['form_mota'],$_POST['button_form_themvasua_danhmuc'])){
        $madanhmuc = $_POST['form_madanhmuc'];
        $tendanhmuc = $_POST['form_tendanhmuc'];
        $mota = $_POST['form_mota'];
    ...
    <main class = "main_danhmuc">
            ...
            <div class = "div_form_danhmuc">
                <form method="post" id = "form_danhmuc">
                    ...
                    <label>Mô tả:</label>
                    <input type="text" name="form_mota" id='ip_mota'>
                    <label>Mã danh mục:</label>
                    <select name="form_selectmadanhmuc" id = "sel_madanhmuc">
                        <option disabled selected>Chọn mã danh mục</option>
                        <?php getMaDanhMucToSelect(); ?> 
                    </select>
...
<script>
    ...
    function xoaForm(){
        ...
        document.getElementById("sel_madanhmuc").disabled=false;
        document.getElementById("form_danhmuc").reset();
        
    }

    function setValueForm(madanhmuc,tendanhmuc, mota){
        document.getElementById("ip_madanhmuc").value=madanhmuc;
        document.getElementById("ip_tendanhmuc").value=tendanhmuc;
        document.getElementById("ip_mota").value=mota;
    }
</script>

<?php
    if(isset($_POST['table_madanhmuc'],$_POST['table_tendanhmuc'],$_POST['table_mota']
                ,$_POST['button_table_chon'])){
        $madanhmuc = $_POST['table_madanhmuc'];
        $tendanhmuc = $_POST['table_tendanhmuc'];
        $mota = $_POST['table_mota'];
        echo "<script> setValueForm('" .$madanhmuc . "','" . $tendanhmuc . "','" . $mota . "')</script>";
    }
?>